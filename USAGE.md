...
- [3. 配置 `.env`](#3-配置-env)
- [4. 快速开始](#4-快速开始)
- [5. 使用方式一：单视频 / URL 处理（推荐）](#5-使用方式一单视频--url-处理推荐)
- [6. 使用方式二：批量与多课程并行](#6-使用方式二批量与多课程并行)
- [7. 视频下载](#7-视频下载)
- [8. 字幕策略（智能来源链）](#8-字幕策略智能来源链)
- [9. 关键帧检测原理](#9-关键帧检测原理)
- [10. 断点续传与可靠性](#10-断点续传与可靠性)
- [11. 输出文件说明](#11-输出文件说明)
- [12. 测试与调试](#12-测试与调试)
- [13. 常见问题](#13-常见问题)
- [附录 A：Chrome 扩展（可选）](#附录-achrome-扩展可选)
...
**核心数据流**：视频 → ① 音频分段转写（带时间戳）→ ② 智能字幕解析（B站CC → B站AI长视频+校验 → ASR）→ ③ 文字指引关键帧检测 → ④ DeepSeek 总结（英文自动翻译中文）→ ⑤ 写入 Obsidian。
...
# 处理单个 URL → 生成笔记到 Obsidian
python -m server.batch_process "https://www.bilibili.com/video/BVxxx" --url --course "课程名" --mode both
```

> 服务端处理不依赖本地服务常驻（`batch_process` 直接运行）。若用 Chrome 扩展模式才需要启动 uvicorn（见附录 A）。

---

## 5. 使用方式一：单视频 / URL 处理（推荐）

**适用**：已有视频文件，或直接给视频 URL。这是最常用的方式。
...
2. 智能字幕解析：B站CC → B站AI(长视频+双维度校验) → ASR → OCR(可选)
...
```
① B站手工 CC 字幕（yt-dlp 读 Chrome 登录态直接下载，免费最高质量）
② B站/YouTube AI 字幕（仅长视频 >15min 用，省 ASR 成本）
   └ 双维度校验：DeepSeek 语义一致性(ASR vs 字幕) + 合理性
      └ 结果逐句入库，合格率 >95% 自动 trust（此后 ASR/DeepSeek 只做审查）
③ ASR 语音转写（默认兜底）
④ OCR 字幕（可选降级，SUBTITLE_SOURCE=ocr 启用）
```

### 配置

```ini
SUBTITLE_SOURCE=auto        # auto(智能链) | bili | asr | ocr | off
SUBTITLE_AI_MIN_SEC=900     # 长视频阈值（秒），>15min 才考虑 AI 字幕
SUBTITLE_VERIFY=true        # 双维度校验 AI 字幕质量
SUBTITLE_TRUST_THRESHOLD=0.95  # 合格率阈值，>95% 大规模采用 AI 字幕
```

### 自适应决策
- **合格率 ≤95%**：每次用 AI 字幕都做双维度校验（ASR + DeepSeek），结果入库
- **合格率 >95%**：该来源字幕可信任，大规模采用；ASR/DeepSeek 只做抽查审查
- 长视频阈值可调（如日后效果好可全部用 AI 字幕）

### 并发安全
- 字幕抓取用 **yt-dlp**（读 Chrome 登录态），每个 worker 独立并行，**不加锁**
- opencli 仅用于**视频下载兜底**（yt-dlp 失败时），此处才加文件锁串行
- 视频并行下载不受影响
...
> 主要使用方式为脚本（见第 5/6 节）。Chrome 扩展用于「边看视频边实时积累字幕 + 截图」的场景，为可选附加功能。

**加载**：
1. 打开 `chrome://extensions`
2. 开启「开发者模式」
3. 「加载已解压的扩展程序」→ 选择 `extension/` 目录
4. 快捷键：`⌘⇧E` 开始/停止学习、`⌘⇧S` 截图

**使用**：
1. **启动本地服务**：`uvicorn server.app:app --host 127.0.0.1 --port 8787`（扩展依赖它）
2. 打开任意含视频的网页（B 站、飞书等）
3. 点扩展图标 → 「开始学习」（或按 `⌘⇧E`）
4. 后台自动积累字幕、转写音频、检测关键画面；`⌘⇧S` 手动截图
5. 视频播完自动生成学习文档
