...
- **🎯 智能字幕来源**：按可插拔来源链生成字幕——**B站手工 CC 字幕**（yt-dlp 读 Chrome 登录态直接下载）→ **B站/YouTube AI 字幕**（长视频 >15min 才用，省 ASR 成本）→ **ASR 语音转写兜底** → **OCR**（可选降级）。长视频用 AI 字幕时做**双维度质量校验**（DeepSeek 语义一致性 + 合理性），结果逐句入库，**合格率 >95% 自动 trust**（此后 ASR/DeepSeek 只做审查）。并发安全：字幕抓取走 yt-dlp 并行，opencli 仅兜底下载加锁。
